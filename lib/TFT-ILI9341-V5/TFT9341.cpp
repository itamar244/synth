...
void tft9341::print (char const* str)
{
	uint8_t i;
	for (i=0;str[i];i++)
		write(str[i]);
...
void touch::readxy()
{
	...
	{
		this->x = getX();
		this->x = 240 - (this->x - 300) / 14;
		this->y = getY();
		this->y = (this->y - 270) / 10.5;
	}
	else if (TypeTouch == 0)
	{
		this->x = getY();
		this->x = (this->x - 192) / 15.2;
		this->y = getX();
		this->y = (this->y - 192) / 11.4;
	}
	else if (TypeTouch == 2)
	{
		this->x = getX();
		this->x = (this->x - 300) / 14;
		this->y = getY();
		this->y = 320 + 20 - (this->y - 270) / 10.5;
	}
/*
	if (rotation==0){
